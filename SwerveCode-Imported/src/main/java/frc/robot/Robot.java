...
//import frc.robot.ConfigRun; // Robot configuration for driver and autonomous
...
public class Robot extends TimedRobot {
    ...
    public void teleopInit() {
        drive.zeroGyroscope();                  // Set our zero for field-relative driving
    }
    ...
    public void teleopPeriodic() {
        ...
        rotate     = (driverController.getRightX() * driveTrain.MAX_ANGULAR_VELOCITY_RADIANS_PER_SECOND) * ConfigRun.ROTATE_POWER;                                                                                                  // joystick
        translateX = (driverController.getLeftY() * driveTrain.MAX_VELOCITY_METERS_PER_SECOND) * ConfigRun.TRANSLATE_POWER;
        ...
        if (driverController.getRightBumper() == true) {
            drive.drive(ChassisSpeeds.fromFieldRelativeSpeeds(-joystickDeadband(translateX),
                    -joystickDeadband(translateY), -vision.autoAim(), drive.getGyroscopeRotation())); // Inverted due to
                                                                                                      // Robot
                                                                                                      // Directions
                                                                                                      // being the
                                                                                                      // opposite of
            ...
            drive.drive(ChassisSpeeds.fromFieldRelativeSpeeds(-joystickDeadband(translateX),
                    -joystickDeadband(translateY), -joystickDeadband(rotate), drive.getGyroscopeRotation())); // Inverted
                                                                                                              // due to
                                                                                                              // Robot
                                                                                                              // Directions
                                                                                                              // being
